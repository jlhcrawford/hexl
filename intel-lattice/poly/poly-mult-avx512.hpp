...
void MultiplyModInPlaceAVX512(uint64_t* operand1, const uint64_t* operand2,
  ...
  for (size_t i = 0; i < n; i += 8) {
    ...
    vz1 = avx512_multiply_uint64_hi<BitShift>(v_operand1, v_operand2);
    vz0 = _mm512_mullo_epi64(v_operand1, v_operand2);
    // Reduces z using base 2^BitShift Barrett reduction

    // Multiply input and const_ratio
    // Round 1
    vcarry = avx512_multiply_uint64_hi<BitShift>(vz0, vconst_ratio_lo);
    vtmp21 = avx512_multiply_uint64_hi<BitShift>(vz0, vconst_ratio_hi);
    vtmp20 = _mm512_mullo_epi64(vz0, vconst_ratio_hi);

    __m512i vtt = avx512_add_uint64(vtmp20, vcarry, &vtmp1);
    vtmp3 = _mm512_add_epi64(vtmp21, vtt);

    // Round 2
    vtmp21 = avx512_multiply_uint64_hi<BitShift>(vz1, vconst_ratio_lo);
    vtmp20 = _mm512_mullo_epi64(vz1, vconst_ratio_lo);
